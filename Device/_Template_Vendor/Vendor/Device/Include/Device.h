...
/* <Device>'s External IRQn ID is from the hard-wired persperctive, which has an offset mapped to the ECLIC IRQn.
   eg.: uart0's external interrupt id in <Device> is 32, while its ECLIC IRQn is 51 */
#define SOC_EXTERNAL_MAP_TO_ECLIC_IRQn_OFFSET      19
/* get <Device>'s External IRQn from ECLIC external IRQn which indexs from 19 */
...
#ifdef CFG_HAS_CLIC
/* UART0 Interrupt */
/* NOTE: Take care the external uart irq may not work, it require a correct <Device> cpu configuration */
...
// it is defined in system_<Device>.c, you should not use this variable CpuIRegionBase
// SystemIRegionInfo variable in previous release is removed, you should avoid to use it
// you should use macro __IREGION_BASEADDR defined in <Device>.h
...
#endif
#ifdef CFG_TMR_BASE_ADDR    // Maybe using timer out of cpu for <Device>
...
// RUNMODE_* macros are defined in SoC/<Device>/runmode.mk, for internal usage not intend for widely usage
#ifdef RUNMODE_IC_EN
...
#ifdef <Device>_RTC_FREQ
#define RTC_FREQ                    <Device>_RTC_FREQ
#else
...
#ifndef <Device>_FLASH_XIP_BASE
#define QSPI_FLASH_BASE         (0x20000000UL)                /*!< (FLASH     ) Base Address */
#else
#define QSPI_FLASH_BASE         (<Device>_FLASH_XIP_BASE)      /*!< (FLASH     ) Base Address */
#endif
...
#ifndef <Device>_SYSMEM_BASE
#define ONCHIP_SRAM_BASE         (0xA0000000UL)                /*!< (SRAM       ) Base Address */
#else
#define ONCHIP_SRAM_BASE         (<Device>_SYSMEM_BASE)          /*!< (SRAM       ) Base Address */
#endif

#ifndef <Device>_PERIPS_BASE
#define <Device>_PERIPH_BASE     (0x10000000UL)                /*!< (Peripheral) Base Address */
#else
#define <Device>_PERIPH_BASE     (<Device>_PERIPS_BASE)         /*!< (Peripheral) Base Address */
#endif

/* Peripheral memory map */
#define UART0_BASE              (<Device>_PERIPH_BASE + 0x13000)          /*!< (UART0) Base Address */
#define QSPI0_BASE              (<Device>_PERIPH_BASE + 0x14000)          /*!< (QSPI0) Base Address */
#define UART1_BASE              (<Device>_PERIPH_BASE + 0x23000)          /*!< (UART1) Base Address */
#define QSPI1_BASE              (<Device>_PERIPH_BASE + 0x24000)          /*!< (QSPI1) Base Address */
#define QSPI2_BASE              (<Device>_PERIPH_BASE + 0x34000)          /*!< (QSPI2) Base Address */
