...
{
    float16_t biquad_cascade_stereo_df2T_f16_output[2 * TEST_LENGTH_SAMPLES];
    float16_t IIRStateSteF16[4 * numStages];

    riscv_biquad_cascade_stereo_df2T_instance_f16 S;
    generate_rand_f16(testInput_f16_50Hz_200Hz, 2 * TEST_LENGTH_SAMPLES);
    riscv_float_to_f16(IIRCoeffs32LP, IIRCoeffsF16LP, 5 * numStages);

    riscv_biquad_cascade_stereo_df2T_init_f16(&S, numStages, IIRCoeffsF16LP, IIRStateSteF16);
    BENCH_START(riscv_biquad_cascade_stereo_df2T_f16);
    riscv_biquad_cascade_stereo_df2T_f16(&S, testInput_f16_50Hz_200Hz, biquad_cascade_stereo_df2T_f16_output, TEST_LENGTH_SAMPLES);
