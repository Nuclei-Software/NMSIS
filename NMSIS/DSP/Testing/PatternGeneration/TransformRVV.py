...
def cifft(d):
    ...
    if d == "F16":
        return ""
    return f"""    void TransformRVVC{d}::test_cifft_{d.lower()}()
    {{
...
""",file=c)
        for d in DATATYPES:
                ...
                print(f"""               class = TransformRVVC{d}
               folder = TransformRVV{d}
...
def the_test(d):
    ...
    void TransformRVVC{d}::test_cfft_{d.lower()}()
    {{
...
def test_specific_init(d):
    ...
    if d == "F16":
        return ""
    return f"tmp2.create(ref.nbSamples(),TransformRVVC{d}::OUTPUT_CFFT_{d}_ID,mgr);"

if args.c:
    for d in DATATYPES:
        path=os.path.join(args.others[0],f"TransformRVVC{d}.cpp")
        with open(path,"w") as c:
             print(f"""#if defined(RISCV_MATH_VECTOR)
...
""",file=c)
             ...
             print(f"""
   void TransformRVVC{d}::setUp(Testing::testID_t id,std::vector<Testing::param_t>& paramsArgs,Client::PatternMgr *mgr)
   {{
...
""",file=c)
             ...
             for l in fftsizes(d):
                nb = nb + 1
                print(f"""
         case TransformRVVC{d}::TEST_CFFT_{d}_{nb}:

            input.reload(TransformRVVC{d}::INPUTS_CFFT_NOISY_{l}_{d}_ID,mgr);
            ref.reload(  TransformRVVC{d}::REF_CFFT_NOISY_{l}_{d}_ID,mgr);

            this->varInstCfft{d}Ptr=riscv_cfft_init_dynamic_{d.lower()}({l});
...
""",file=c)
         ...
         case TransformRVVC{d}::TEST_CFFT_{d}_{nb}:
            ...
            input.reload(TransformRVVC{d}::INPUTS_CFFT_STEP_{l}_{d}_ID,mgr);
            ref.reload(  TransformRVVC{d}::REF_CFFT_STEP_{l}_{d}_ID,mgr);
...
""",file=c)
             for l in fftsizes(d):
                nb = nb + 1
                print(f"""
         case TransformRVVC{d}::{ifft_testname(d).upper()}{d}_{nb}:

            input.reload(TransformRVVC{d}::INPUTS_CIFFT_NOISY_{l}_{d}_ID,mgr);
            ref.reload(  TransformRVVC{d}::INPUTS_CFFT_NOISY_{l}_{d}_ID,mgr);
...
""",file=c)
         ...
         case TransformRVVC{d}::{ifft_testname(d).upper()}{d}_{nb}:
            ...
            input.reload(TransformRVVC{d}::INPUTS_CIFFT_STEP_{l}_{d}_ID,mgr);
            ref.reload(  TransformRVVC{d}::INPUTS_CFFT_STEP_{l}_{d}_ID,mgr);
...
""",file=c)
       ...
       inputfft.create(ref.nbSamples(),TransformRVVC{d}::OUTPUT_CFFT_{d}_ID,mgr);

       outputfft.create(ref.nbSamples(),TransformRVVC{d}::OUTPUT_CFFT_{d}_ID,mgr);
       bufferfft.create(ref.nbSamples(),TransformRVVC{d}::OUTPUT_CFFT_{d}_ID,mgr);
       {test_specific_init(d)}

    }}

    void TransformRVVC{d}::tearDown(Testing::testID_t id,Client::PatternMgr *mgr)
    {{
        (void)id;
        outputfft.dump(mgr);
