...
def writeTests(config,format):
    ...
    if format==31:
        ref = ref / 2**15 # Because NMSIS format is 16.48
        config.writeReferenceQ63(2, asReal(ref))
    elif format==15:
        ref = ref / 2**7 # Because NMSIS format is 8.24
...
#
    ...
    if format==31:
        ref = ref / 2 # Because NMSIS format is 2.30
    elif format==15:
        ref = ref / 2 # Because NMSIS format is 2.14
    config.writeReference(5, ref)
#
    ref = np.absolute(data1)**2
    if format==31:
        ref = ref / 4 # Because NMSIS format is 3.29
    elif format==15:
        ref = ref / 4 # Because NMSIS format is 3.13
