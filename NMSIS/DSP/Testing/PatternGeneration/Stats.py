...
# computed by NMSIS which is normal but then the NMSIS test will fail.
...
def powerTest(format,data):
   if format == 31:
       return(np.dot(data,data) / 2**15) # NMSIS is 2.28 format
   elif format == 15:
       return(np.dot(data,data) / 2**33) # NMSIS is 34.30 format
   elif format == 7:
       return(np.dot(data,data) / 2**17) # NMSIS is 18.14 format
   else:
...
def writeTests(config,nb,format):
    ...
    # in min/max is coherent between Python and NMSIS.
