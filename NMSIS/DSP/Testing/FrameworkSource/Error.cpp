...
#if !defined( __CC_ARM ) && defined(RISCV_FLOAT16_SUPPORTED)
float riscv_snr_f16(float16_t *pRef, float16_t *pTest, uint32_t buffSize)
{
  float EnergySignal = 0.0, EnergyError = 0.0;
  uint32_t i;
  float SNR;
...
float riscv_snr_q63(q63_t *pRef, q63_t *pTest, uint32_t buffSize)
{
...
float riscv_snr_q31(q31_t *pRef, q31_t *pTest, uint32_t buffSize)
{
  float EnergySignal = 0.0, EnergyError = 0.0;
  uint32_t i;
  float SNR;
...
float riscv_snr_q15(const q15_t *pRef, const q15_t *pTest, uint32_t buffSize)
{
  float EnergySignal = 0.0, EnergyError = 0.0;
  uint32_t i;
  float SNR;
...
float riscv_snr_q7(q7_t *pRef, q7_t *pTest, uint32_t buffSize)
{
  float EnergySignal = 0.0, EnergyError = 0.0;
  uint32_t i;
  float SNR;
...
double riscv_snr_f64(double *pRef, double *pTest, uint32_t buffSize)
{
  double EnergySignal = 0.0, EnergyError = 0.0;
  uint32_t i;
  double SNR;
...
void assert_snr_error(unsigned long nb,AnyPattern<q63_t> &pa,AnyPattern<q63_t> &pb, float32_t threshold)
{
   ...
   snr = riscv_snr_q63(ptrA, ptrB, pa.nbSamples());
...
void assert_snr_error(unsigned long nb,AnyPattern<q31_t> &pa,AnyPattern<q31_t> &pb, float32_t threshold)
{
   ...
   snr = riscv_snr_q31(ptrA, ptrB, pa.nbSamples());
  ...
  unsigned long nb)
{
  ...
  snr = riscv_snr_q15(ptrA, ptrB, nb);
...
void assert_snr_error(unsigned long nb,AnyPattern<q15_t> &pa,AnyPattern<q15_t> &pb, float32_t threshold)
{
   ...
   snr = riscv_snr_q15(ptrA, ptrB, pa.nbSamples());
...
void assert_snr_error(unsigned long nb,AnyPattern<q7_t> &pa,AnyPattern<q7_t> &pb, float32_t threshold)
{
   ...
   snr = riscv_snr_q7(ptrA, ptrB, pa.nbSamples());
