...
*/
    ...
    void StatsTestsF64::test_entropy_f64()
    {
      ...
      {
         *outp = riscv_entropy_f64(inp,dimsp[i+1]);
         outp++;
         inp += dimsp[i+1];
      }
...
/*
    void StatsTestsF64::test_logsumexp_f64()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_logsumexp_f64(inp,dimsp[i+1]);
         outp++;
         inp += dimsp[i+1];
      }
...
*/
    void StatsTestsF64::test_kullback_leibler_f64()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_kullback_leibler_f64(inpA,inpB,dimsp[i+1]);
         outp++;
         inpA += dimsp[i+1];
         inpB += dimsp[i+1];
      }
...
/*
    void StatsTestsF64::test_logsumexp_dot_prod_f64()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_logsumexp_dot_prod_f64(inpA,inpB,dimsp[i+1],tmpp);
         outp++;
         inpA += dimsp[i+1];
         inpB += dimsp[i+1];
      }
...
*/
    ...
    void StatsTestsF64::test_accumulate_f64()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         riscv_accumulate_f64(inp,dimsp[i+1],outp);
         outp++;
      }
