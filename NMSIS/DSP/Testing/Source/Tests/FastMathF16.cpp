...
#define ABS_ERROR_ATAN (2.0e-3)
    ...
    void FastMathF16::test_atan2_scalar_f16()
    {
        ...
        for(i=0; i < ref.nbSamples(); i++)
        {
          status=riscv_atan2_f16(inp[2*i],inp[2*i+1],&res);
          outp[i]=res;
    ...
    void FastMathF16::test_sqrt_f16()
    {
        ...
        for(i=0; i < ref.nbSamples(); i++)
        {
           status=riscv_sqrt_f16(inp[i],&outp[i]);
           ASSERT_TRUE((status == RISCV_MATH_SUCCESS) || (((float32_t)inp[i] < 0.0f) && (status == RISCV_MATH_ARGUMENT_ERROR)));
        }
    ...
    void FastMathF16::test_vlog_f16()
    {
        ...
        riscv_vlog_f16(inp,outp,ref.nbSamples());
    ...
    void FastMathF16::test_inverse_f16()
    {
        ...
        riscv_vinverse_f16(inp,outp,ref.nbSamples());
