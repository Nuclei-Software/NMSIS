...
#define ABS_ERROR_ATAN (5.0e-7)
    ...
    void FastMathF32::test_atan2_scalar_f32()
    {
        ...
        {
          status=riscv_atan2_f32(inp[2*i],inp[2*i+1],&res);
          outp[i]=res;
    ...
    void FastMathF32::test_sqrt_f32()
    {
        ...
        for(i=0; i < ref.nbSamples(); i++)
        {
           status=riscv_sqrt_f32(inp[i],&outp[i]);
           ASSERT_TRUE((status == RISCV_MATH_SUCCESS) || ((inp[i] < 0.0f) && (status == RISCV_MATH_ARGUMENT_ERROR)));
        }
