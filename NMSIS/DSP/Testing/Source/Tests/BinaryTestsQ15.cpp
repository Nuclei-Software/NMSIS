...
#define PREPAREDATA2R()                                                   \
    ...
    {     
      ...
      for(i=0;i < nbMatrixes ; i ++)
      {
          ...
          try
          {
             memset(tmpPtr,0,sizeof(q15_t)*internal*columns + 16);
             if (shift!=0)
               riscv_shift_q15(inp1,-shift,ap,rows*internal);
             status=riscv_mat_mult_fast_q15(&this->in1,&this->in2,&this->out,tmpPtr);
             if (shift!=0)
    ...
    {     
      ...
      for(i=0;i < nbMatrixes ; i ++)
      {
          ...
          status=riscv_mat_cmplx_mult_q15(&this->in1,&this->in2,&this->out,tmpPtr);
          ASSERT_TRUE(status==RISCV_MATH_SUCCESS);
