...
#define REL_ERROR (1.0e-5)
    ...
    void StatsTestsF32::test_entropy_f32()
    {
      ...
      {
         *outp = riscv_entropy_f32(inp,dimsp[i+1]);
         outp++;
         inp += dimsp[i+1];
      }
    ...
    void StatsTestsF32::test_logsumexp_f32()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_logsumexp_f32(inp,dimsp[i+1]);
         outp++;
         inp += dimsp[i+1];
      }
    ...
    void StatsTestsF32::test_kullback_leibler_f32()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_kullback_leibler_f32(inpA,inpB,dimsp[i+1]);
         outp++;
         inpA += dimsp[i+1];
         inpB += dimsp[i+1];
      }
    ...
    void StatsTestsF32::test_logsumexp_dot_prod_f32()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_logsumexp_dot_prod_f32(inpA,inpB,dimsp[i+1],tmpp);
         outp++;
         inpA += dimsp[i+1];
         inpB += dimsp[i+1];
      }
    ...
    void StatsTestsF32::test_accumulate_f32()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         riscv_accumulate_f32(inp,dimsp[i+1],outp);
         outp++;
      }
