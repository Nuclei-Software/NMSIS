...
#include "Test.h"
    ...
    void DistanceTestsF32::test_dtw_distance_f32()
    {
       ...
       riscv_matrix_instance_f32 distances;
       riscv_matrix_instance_f32 costs;
       riscv_matrix_instance_q7 window;
       ...
       for(int i=0; i < this->nbPatterns ; i ++)
       {
          ...
          riscv_dtw_path_f32(&costs,outPathp,&pathLength);

          /* RISCV_DTW_SAKOE_CHIBA_WINDOW 5*/
          status = riscv_dtw_init_window_q7(RISCV_DTW_SAKOE_CHIBA_WINDOW,5,&window);
          ASSERT_TRUE(status == RISCV_MATH_SUCCESS);
          ...
          status = riscv_dtw_distance_f32(&distances, &window,&costs,outp);
          ASSERT_TRUE(status == RISCV_MATH_SUCCESS);
          outp++;

          /* RISCV_DTW_SAKOE_CHIBA_WINDOW 3 */
          status = riscv_dtw_init_window_q7(RISCV_DTW_SAKOE_CHIBA_WINDOW,3,&window);
          ASSERT_TRUE(status == RISCV_MATH_SUCCESS);
          status = riscv_dtw_distance_f32(&distances, &window,&costs,outp);
          ASSERT_TRUE(status == RISCV_MATH_ARGUMENT_ERROR);

          /* RISCV_DTW_SLANTED_BAND_WINDOW 1*/
          status = riscv_dtw_init_window_q7(RISCV_DTW_SLANTED_BAND_WINDOW,1,&window);
          ASSERT_TRUE(status == RISCV_MATH_SUCCESS);
