...
#define ABS_KULLBACK_ERROR (1.0e-2)
    ...
    void StatsTestsF16::test_max_f16()
    {
        ...
        ASSERT_EQ((float32_t)result,(float32_t)refp[this->refOffset]);
    ...
    void StatsTestsF16::test_std_stability_f16()
    {
      ...
      The NMSIS-DSP variance algorithm is the two pass one so will work
      with those values.
      ...
      */
      ASSERT_TRUE(fabs(5.47723f - (float32_t)result) < 0.32f);
    ...
    void StatsTestsF16::test_entropy_f16()
    {
      ...
      {
         *outp = riscv_entropy_f16(inp,dimsp[i+1]);
         outp++;
         inp += dimsp[i+1];
      }
    ...
    void StatsTestsF16::test_logsumexp_f16()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_logsumexp_f16(inp,dimsp[i+1]);
         outp++;
         inp += dimsp[i+1];
      }
    ...
    void StatsTestsF16::test_kullback_leibler_f16()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_kullback_leibler_f16(inpA,inpB,dimsp[i+1]);
         outp++;
         inpA += dimsp[i+1];
         inpB += dimsp[i+1];
      }
    ...
    void StatsTestsF16::test_logsumexp_dot_prod_f16()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         *outp = riscv_logsumexp_dot_prod_f16(inpA,inpB,dimsp[i+1],tmpp);
         outp++;
         inpA += dimsp[i+1];
         inpB += dimsp[i+1];
      }
    ...
    void StatsTestsF16::test_mse_f16()
    {
        ...
        ASSERT_REL_ERROR(result,refp[this->refOffset],(double)REL_ERROR);
    ...
    void StatsTestsF16::test_accumulate_f16()
    {
      ...
      for(int i=0;i < this->nbPatterns; i++)
      {
         riscv_accumulate_f16(inp,dimsp[i+1],outp);
         outp++;
      }
