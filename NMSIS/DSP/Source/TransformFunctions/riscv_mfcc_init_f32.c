...
RISCV_DSP_ATTRIBUTE riscv_status riscv_mfcc_init_f32(
  ...
  )
{
 ...
 status=riscv_cfft_init_f32(&(S->cfft),fftLen);
 #else
...
RISCV_DSP_ATTRIBUTE riscv_status riscv_mfcc_init_##LEN##_f32(         \
  riscv_mfcc_instance_f32 * S,                  \
  uint32_t nbMelFilters,                      \
  uint32_t nbDctOutputs,                      \
  const float32_t *dctCoefs,                  \
...
{                                             \
 ...
 S->windowCoefs=windowCoefs;                  \
                                              \
 status=riscv_cfft_init_##LEN##_f32(&(S->cfft));\
                                              \
 return(status);                              \
}
#else
...
{                                                  \
 ...
 S->windowCoefs=windowCoefs;                       \
                                                   \
 status=riscv_rfft_fast_init_##LEN##_f32(&(S->rfft));\
                                                   \
 return(status);                                   \
}
#endif
