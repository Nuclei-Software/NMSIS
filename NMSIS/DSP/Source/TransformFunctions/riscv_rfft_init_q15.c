...
#define RFFTINIT_Q15(LEN, CFFTLEN, TWIDMOD)                                    \
    ...
    {                                                                          \
        riscv_status status;                                                   \
        status = riscv_cfft_init_##CFFTLEN##_q15(&(S->Sint));                  \
        if (status != RISCV_MATH_SUCCESS) {                                    \
            return (status);                                                   \
        }                                                                      \
        S->fftLenRFFT = LEN;                                                   \
        S->ptwd_re = riscv_rvv_rfft_twdre_##LEN##_q15;                         \
        S->ptwd_im = riscv_rvv_rfft_twdim_##LEN##_q15;                         \
...
{
    riscv_rfft_instance_q15 *S = (riscv_rfft_instance_q15 *)malloc(sizeof(riscv_rfft_instance_q15));
    if (S != NULL) {
        riscv_rfft_init_q15(S, fftLenReal);
    }
    return S;
}
    ...
    uint32_t fftLenReal)
{
    ...
    {
    case 8192U:
        status = riscv_rfft_init_8192_q15( S );
        break;
    case 4096U:
        status = riscv_rfft_init_4096_q15( S );
        break;
    case 2048U:
        status = riscv_rfft_init_2048_q15( S );
        break;
    case 1024U:
        status = riscv_rfft_init_1024_q15( S );
        break;
    case 512U:
        status = riscv_rfft_init_512_q15( S );
        break;
    case 256U:
        status = riscv_rfft_init_256_q15( S );
        break;
    case 128U:
        status = riscv_rfft_init_128_q15( S );
        break;
    case 64U:
        status = riscv_rfft_init_64_q15( S );
        break;
   case 32U:
        status = riscv_rfft_init_32_q15( S );
        break;
    default:
