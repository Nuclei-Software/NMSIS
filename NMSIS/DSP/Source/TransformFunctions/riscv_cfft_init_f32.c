...
#define CFFTINIT_F32(LEN, LENTWIDDLE)                                          \
    RISCV_DSP_ATTRIBUTE riscv_status riscv_cfft_init_##LEN##_f32(              \
        riscv_cfft_instance_f32 *S)                                            \
    {                                                                          \
        /*  Initialise the default riscv status */                             \
        riscv_status status = RISCV_MATH_SUCCESS;                              \
        S->fftLen = LEN;                                                       \
        S->ptwd_re = (const float32_t *)riscv_rvv_twdre_##LEN##_f32;           \
        S->ptwd_im = (const float32_t *)riscv_rvv_twdim_##LEN##_f32;           \
        S->pBitRevTable = riscv_rvv_bitrev_##LEN##_f32;                        \
        return status;                                                         \
    }

riscv_cfft_instance_f32 *riscv_cfft_init_dynamic_f32(uint32_t fftLen)
{
    riscv_cfft_instance_f32 *S = (riscv_cfft_instance_f32 *)malloc(sizeof(riscv_cfft_instance_f32));
    if (S != NULL) {
        riscv_cfft_init_f32(S, fftLen);
    }
    return S;
}
