...
riscv_status riscv_mfcc_q15(const riscv_mfcc_instance_q15 *S, q15_t *pSrc,
                            q15_t *pDst, q31_t *pTmp)
{
    ...
    q31_t logExponent;
    q63_t result;
    riscv_matrix_instance_q15 pDctMat;
    uint32_t i;
    uint32_t coefsPos;
    ...
    if (m != 0) {
        q15_t quotient;
        int16_t shift;
        status = riscv_divide_q15(0x7FFF, m, &quotient, &shift);
    ...
    */
    ...
    riscv_cfft_q15(&(S->cfft), pTmp2, 0, 1);
#else
    ...
    for (i = 0; i < S->nbMelFilters; i++) {
        riscv_dot_prod_q15(pSrc + S->filterPos[i], &(S->filterCoefs[coefsPos]),
