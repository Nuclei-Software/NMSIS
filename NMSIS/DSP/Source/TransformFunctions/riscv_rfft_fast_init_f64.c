...
/**
  ...
  @par
                If you use NMSIS-DSP as a library, and if you know the FFT sizes
                that you need at build time, then it is better to use the initialization
                functions defined for each FFT size.

 */

RISCV_DSP_ATTRIBUTE riscv_status riscv_rfft_fast_init_f64(
  ...
  uint16_t fftLen)
{
  ...
  {
  case 4096U:
    status = riscv_rfft_fast_init_4096_f64(S);
    break;
  case 2048U:
    status = riscv_rfft_fast_init_2048_f64(S);
    break;
  case 1024U:
    status = riscv_rfft_fast_init_1024_f64(S);
    break;
  case 512U:
    status = riscv_rfft_fast_init_512_f64(S);
    break;
  case 256U:
    status = riscv_rfft_fast_init_256_f64(S);
    break;
  case 128U:
    status = riscv_rfft_fast_init_128_f64(S);
    break;
  case 64U:
    status = riscv_rfft_fast_init_64_f64(S);
    break;
  case 32U:
    status = riscv_rfft_fast_init_32_f64(S);
    break;
  default:
