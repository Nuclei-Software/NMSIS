...
__STATIC_FORCEINLINE q15_t riscv_atan_limited_q15(q15_t x)
{
...
__STATIC_FORCEINLINE q15_t riscv_atan_q15(q15_t y,q15_t x)
{
   int sign=0;
   q15_t res=0;
   ...
   if (y > x)
   {
    ...
    riscv_divide_q15(x,y,&ratio,&shift);

    riscv_shift_q15(&ratio,shift,&ratio,1);
   ...
   else
   {
    ...
    riscv_divide_q15(y,x,&ratio,&shift);
...
riscv_status riscv_atan2_q15(q15_t y,q15_t x,q15_t *result)
{
    ...
    if (x < 0)
    {
        if (y > 0)
        {
           *result=riscv_atan_q15(y,x) + PIQ13;
        }
        else if (y < 0)
        {
           *result=riscv_atan_q15(y,x) - PIQ13;
