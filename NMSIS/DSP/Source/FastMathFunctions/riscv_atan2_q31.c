...
__STATIC_FORCEINLINE q31_t riscv_atan_limited_q31(q31_t x)
{
...
__STATIC_FORCEINLINE q31_t riscv_atan_q31(q31_t y,q31_t x)
{
   int sign=0;
   q31_t res=0;
...
riscv_status riscv_atan2_q31(q31_t y,q31_t x,q31_t *result)
{
    ...
    if (x < 0)
    {
        if (y > 0)
        {
           *result=riscv_atan_q31(y,x) + PIQ29;
        }
        else if (y < 0)
        {
           *result=riscv_atan_q31(y,x) - PIQ29;
