...
__STATIC_FORCEINLINE float32_t riscv_atan_limited_f32(float32_t x)
{
...
__STATIC_FORCEINLINE float32_t riscv_atan_f32(float32_t x)
{
   int sign=0;
   float32_t res=0.0f;
   ...
   {
      x = 1.0f / x;
      res = PIHALFF32 - riscv_atan_limited_f32(x);
   }
   else
   {
     res += riscv_atan_limited_f32(x);
   }
...
RISCV_DSP_ATTRIBUTE riscv_status riscv_atan2_f32(float32_t y,float32_t x,float32_t *result)
{
    if (x > 0.0f)
    {
        *result=riscv_atan_f32(y/x);
