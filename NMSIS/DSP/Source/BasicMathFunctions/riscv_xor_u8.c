...
RISCV_DSP_ATTRIBUTE void riscv_xor_u8(
    const uint8_t * pSrcA,
    const uint8_t * pSrcB,
          uint8_t * pDst,
          uint32_t blockSize)
{
  ...
  {
    vx = __riscv_vle8_v_u8m8(pSrcA, l);
    pSrcA += l;
    vy = __riscv_vle8_v_u8m8(pSrcB, l);
    pSrcB += l;
    __riscv_vse8_v_u8m8(pDst, __riscv_vxor_vv_u8m8(vx, vy, l), l);
    pDst += l;
  }
#else
