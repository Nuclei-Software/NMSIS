...
RISCV_DSP_ATTRIBUTE void riscv_lms_norm_q15(
  ...
  while (blkCnt > 0U)
  {
    ...
    *pStateCurnt++ = *pSrc;
    ...
    while (tapCnt > 0U)
    {
      ...
      acc = __SMLALD(read_q15x4_ia (&px), read_q15x4_ia (&pb), acc);
#else
    ...
    *pErr++ = e;
    ...
    for (; (vl = __riscv_vsetvl_e16m4(vblkCnt1)) > 0; vblkCnt1 -= vl) {
      ...
      __riscv_vse16_v_i16m4(pb, __riscv_vnclip_wx_i16m4(__riscv_vwadd_wv_i32m8(__riscv_vsra_vx_i32m8(__riscv_vwmul_vx_i32m8(vx1, w, vl), 15, vl), vy1, vl), 0, __RISCV_VXRM_RNU, vl), vl);
      pb += vl;
    }
#else
#if defined (RISCV_MATH_LOOPUNROLL)
