 ...
 */
RISCV_DSP_ATTRIBUTE void riscv_fir_decimate_f32(
        ...
        uint32_t blockSize)
{
...
#endif /* defined (RISCV_MATH_LOOPUNROLL) && !defined (RISCV_MATH_VECTOR_ZVE32F) */
  ...
  for (; (l = __riscv_vsetvl_e32m8(blkCnti)) > 0; blkCnti -= l) {
    __riscv_vse32_v_f32m8(pStateCur, __riscv_vle32_v_f32m8(pSrc, l), l);
    pSrc += l;
    pStateCur += l;
  }
#else
  ...
  for (; (l = __riscv_vsetvl_e32m8(blkCnti)) > 0; blkCnti -= l) {
    __riscv_vse32_v_f32m8(pStateCur, __riscv_vle32_v_f32m8(pState, l), l);
    pState += l;
    pStateCur += l;
  }
#else
