...
RISCV_DSP_ATTRIBUTE float32_t riscv_euclidean_distance_f32(const float32_t *pA,const float32_t *pB, uint32_t blockSize)
{
   ...
   for (; (l = __riscv_vsetvl_e32m8(blkCnt)) > 0; blkCnt -= l) {
      ...
      v_sum = __riscv_vfmacc_vv_f32m8_tu(v_sum, v_at, v_at, l);
   }
   l = __riscv_vsetvl_e32m8(1);
   vfloat32m1_t v_temp = __riscv_vfmv_v_f_f32m1(0.0f, l);
   l = __riscv_vsetvlmax_e32m8();
   v_temp = __riscv_vfredosum_vs_f32m8_f32m1(v_sum, v_temp, l);
   accum += __riscv_vfmv_f_s_f32m1_f32(v_temp);
#else
   while(blockSize > 0)
   {
      tmp = *pA++ - *pB++;
