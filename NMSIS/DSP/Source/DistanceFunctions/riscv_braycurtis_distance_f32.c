...
RISCV_DSP_ATTRIBUTE float32_t riscv_braycurtis_distance_f32(const float32_t *pA,const float32_t *pB, uint32_t blockSize)
{
   ...
   for (; (l = __riscv_vsetvl_e32m8(blkCnt)) > 0; blkCnt -= l) {
      ...
      v_sumdiff = __riscv_vfadd_vv_f32m8_tu(v_sumdiff, __riscv_vfabs_v_f32m8(v_at, l), v_sumdiff, l);
      v_bt = __riscv_vfadd_vv_f32m8(v_x, v_y, l);
      v_sum = __riscv_vfadd_vv_f32m8_tu(v_sum, __riscv_vfabs_v_f32m8(v_bt, l), v_sum, l);
   }
