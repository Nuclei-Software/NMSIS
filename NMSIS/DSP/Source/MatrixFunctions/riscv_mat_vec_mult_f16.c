...
RISCV_DSP_ATTRIBUTE void riscv_mat_vec_mult_f16(const riscv_matrix_instance_f16 *pSrcMat, const float16_t *pVec, float16_t *pDst)
{
    ...
    while (row > 0) {
        ...
        while (colCnt > 0u) {
            ...
            matData = *(pInA1)++;
            sum1 += (float16_t)matData * (float16_t)vecData;
            matData = *(pInA2)++;
            sum2 += (float16_t)matData * (float16_t)vecData;
            matData = *(pInA3)++;
            sum3 += (float16_t)matData * (float16_t)vecData;
            matData = *(pInA4)++;
            sum4 += (float16_t)matData * (float16_t)vecData;
