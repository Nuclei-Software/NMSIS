...
void riscv_absmin_no_idx_q15(const q15_t *pSrc, uint32_t blockSize,
                             q15_t *pResult)
{
    ...
    while (blkCnt > 0U) {
        cur_absmin = *pSrc++;
        cur_absmin =
            (cur_absmin > 0) ? cur_absmin : (q15_t)__QSUB16(0, cur_absmin);
        if (cur_absmin < out) {
            out = cur_absmin;
        }
        /* Decrement loop counter */
                             ...
                             q15_t *pResult)
{
    ...
    for (; (l = vsetvl_e16m8(blkCnt)) > 0; blkCnt -= l) {
        ...
        minVal = vmv_x_s_i16m1_i16(vredmin_vs_i16m8_i16m1(v_temp, v_x, v_temp, l));
        if (minVal < out) {
            out = minVal;
        }
    }
