 ...
 */
RISCV_DSP_ATTRIBUTE void riscv_min_q15(
  const q15_t * pSrc,
        uint32_t blockSize,
        q15_t * pResult,
        uint32_t * pIndex)
{
  ...
  for (; (l = __riscv_vsetvl_e16m8(blkCnt)) > 0; blkCnt -= l)
  {
      ...
      inputx += l;
      min_temp = __riscv_vmv_x_s_i16m1_i16(__riscv_vredmin_vs_i16m8_i16m1(v_x, v_tempa, l));
      if (min_temp < out) {
        out = min_temp;
        mask = __riscv_vmseq_vx_i16m8_b2(v_x, min_temp, l);
