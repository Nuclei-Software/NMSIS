 ...
 */
RISCV_DSP_ATTRIBUTE void riscv_power_q31(
  const q31_t * pSrc,
        uint32_t blockSize,
        q63_t * pResult)
{
  ...
  for (; (l = __riscv_vsetvl_e32m4(blkCnt)) > 0; blkCnt -= l)
  {
    ...
    input += l;
    v_in2 = __riscv_vsra_vx_i64m8(__riscv_vwmul_vv_i64m8(v_in, v_in, l), 14, l);
    v_sum = __riscv_vredsum_vs_i64m8_i64m1(v_in2, v_sum, l);
  }
