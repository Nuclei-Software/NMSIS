...
void fully_connected_riscv_fully_connected_s8(void)
{
    ...
    int32_t *buf = ctx.buf;
    TEST_ASSERT_EQUAL(expected, riscv_vector_sum_s8(buf, filter_dims.n, output_dims.c, kernel_data));
#endif

    riscv_nmsis_nn_status result = riscv_fully_connected_s8(&ctx,
                                                        &fc_params,
                                                        &quant_params,
                                                        &input_dims,
                                                        input_data,
...
void fully_connected_mve_1_riscv_fully_connected_s8(void)
{
    const riscv_nmsis_nn_status expected = RISCV_NMSIS_NN_SUCCESS;
...
void fully_connected_null_bias_0_riscv_fully_connected_s8(void)
{
    const riscv_nmsis_nn_status expected = RISCV_NMSIS_NN_SUCCESS;
    ...
    riscv_nmsis_nn_status ip_check = RISCV_NMSIS_NN_SUCCESS;
    for (int i = 0; i < FULLY_CONNECTED_NULL_BIAS_0_OUT_CH; i++)
    {
        if (bias_data && (bias_data[i] != 0))
        {
            ip_check = RISCV_NMSIS_NN_ARG_ERROR;
            break;
        }
    }
...
void fully_connected_out_activation_riscv_fully_connected_s8(void)
{
    const riscv_nmsis_nn_status expected = RISCV_NMSIS_NN_SUCCESS;
