...
def parse_args():
    parser = argparse.ArgumentParser(description="Run NMSIS-NN unit tests.",
    ...
    parser.add_argument('-s', '--specific-test', type=str, default=None, help="Run a specific test, e.g."
                        " -s TestCases/test_riscv_avgpool_s8 (also this form will work: -s test_riscv_avgpool_s8)."
                        " So basically the different options can be listed with:"
                        " ls -d TestCases/test_* -1")
    parser.add_argument('-c', '--compiler', type=str, default='GCC_RISCV', choices=['GCC_RISCV', 'RISCVC6'])
...
def run_command(command, error_msg=None, die=True):
    # TODO handle error:
    # cp: error writing '/media/mannil01/NODE_F411RE/TESTRUN_NUCLEO_F411RE_GCC_RISCV.bin': No space left on device
...
def test_target(target, args, main_test):
    ...
    try:
        ...
        result = run_command("mbed {} -v -m ".format(mbed_command) + target_model + ' -t ' + compiler +
                             ...
                             ' --source ' + nmsis_nn_path + 'Include/'
                             ' --source ' + nmsis_nn_path + 'Source/ConvolutionFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/PoolingFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/NNSupportFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/FullyConnectedFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/SoftmaxFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/SVDFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/BasicMathFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/ActivationFunctions/'
                             ' --source ' + nmsis_nn_path + 'Source/LSTMFunctions/'
                             + additional_options,
    ...
    for dir in next(os.walk(directory))[1]:
        if re.search(r'test_riscv', dir):
            ...
            for content in os.listdir(testpath):
                if re.search(r'unity_test_riscv', content):
