...
riscv_nmsis_nn_status riscv_lstm_unidirectional_s8_ref(const int8_t *input,
                                               int8_t *output,
                                               ...
                                               nmsis_nn_lstm_context *buffers)
{
    ...
    if (params->time_major)
    {
        ...
        for (int t = 0; t < params->time_steps; t++)
        {
            ...
            riscv_nmsis_nn_status status = riscv_nn_lstm_step_s8_ref(data_in, hidden_in, hidden_out, params, buffers, 1);
    ...
    else
    {
        ...
        for (int t = 0; t < params->time_steps; t++)
        {
            ...
            riscv_nmsis_nn_status status =
                riscv_nn_lstm_step_s8_ref(data_in, hidden_in, hidden_out, params, buffers, params->time_steps);
            if (status != RISCV_NMSIS_NN_SUCCESS)
            {
                return status;
            }
