 ...
 */
riscv_nmsis_nn_status riscv_nn_lstm_step_s8_ref(const int8_t *data_in,
                                        ...
                                        const int32_t batch_offset)
{
    ...
    riscv_nn_lstm_calculate_gate_s8_s16(data_in, hidden_in, &params->forget_gate, params, forget_gate, batch_offset);

    // Calculate first term of cell state in place early to maximise reuse of scratch-buffers
    riscv_elementwise_mul_s16_ref(forget_gate,
                            cell_state,
                            0,
                            0,
                            cell_state,
    ...
    riscv_nn_lstm_calculate_gate_s8_s16_ref(data_in, hidden_in, &params->input_gate, params, input_gate, batch_offset);
    riscv_nn_lstm_calculate_gate_s8_s16_ref(data_in, hidden_in, &params->cell_gate, params, cell_gate, batch_offset);

    // Reminder of cell state calculation, multiply and add to previous result.
    riscv_elementwise_mul_acc_s16_ref(forget_gate,
                                cell_gate,
                                0,
                                0,
                                cell_state,
    ...
    riscv_nn_lstm_calculate_gate_s8_s16_ref(data_in, hidden_in, &params->output_gate, params, output_gate, batch_offset);

    // Calculate hidden state directly to output.
    riscv_nn_activation_s16_ref(
        cell_state, hidden_temp, params->hidden_size * params->batch_size, params->cell_scale_power + 12, RISCV_TANH);
    riscv_elementwise_mul_s16_s8_ref(output_gate,
                               hidden_temp,
                               hidden_out,
