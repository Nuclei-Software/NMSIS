 ...
 * Title:        riscv_depthwise_conv_s4_opt_ref.c
...
riscv_nmsis_nn_status riscv_depthwise_conv_s4_opt_ref(const nmsis_nn_context *ctx,
        ...
        for (int i_out_x = 0; i_out_x < output_x; i_out_x++)
        {
            ...
            else /* Even number of channels */
            {
                ...
                if (output_ch & 0x2)
                {
                    ...
                    sum = MIN(sum, output_activation_max);
                    *output++ = (int8_t)sum;
                    sum2 = riscv_nn_requantize(sum2, *output_mult++, *output_shift++);
                    sum2 += output_offset;
