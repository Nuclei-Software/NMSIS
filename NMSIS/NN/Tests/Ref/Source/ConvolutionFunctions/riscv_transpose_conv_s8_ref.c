 ...
 * Title:        riscv_transpose_conv_s8_ref.c
 ...
 *
 */
riscv_nmsis_nn_status riscv_transpose_conv_s8_ref(const nmsis_nn_context *ctx,
                                          ...
                                          int8_t *output_data)
{
    ...
    const int32_t col_buf_size = riscv_transpose_conv_s8_get_buffer_size_ref(input_dims, filter_dims, output_dims);
    ...
    while (batch_cnt)
    {
        if (bias_data == NULL)
        {
            riscv_memset_s8((int8_t *)img_buf_ptr, 0, output_x * output_y * output_ch * sizeof(int32_t));
        }
        else
        {
        ...
        riscv_memset_s8((int8_t *)col_data_ptr, 0, col_buf_size);

        for (int i_output_ch = 0; i_output_ch < output_ch; i_output_ch++)
        {
            riscv_nn_mat_mult_nt_t_s8_s32_ref(input_data_ptr,
                                        filter_data_ptr,
                                        col_data_ptr,
                                        input_size,
                                        input_ch,
        ...
        for (int i = 0; i < output_x * output_y; i++)
        {
            ...
            {
                int32_t result =
                    riscv_nn_requantize(img_data[i_output_ch], output_multiplier[i_output_ch], output_shift[i_output_ch]);
                result += out_offset;
