...
riscv_max_no_broadcast_s8(const int8_t *input_1, const int8_t *input_2, int8_t *output, int32_t flat_size)
{
    while (flat_size > 0)
    {
        int8_t in1 = *input_1++;
...
static riscv_nmsis_nn_status
riscv_max_scalar_s8(const int8_t *input_1, const int8_t *input_2, int8_t *output, int32_t flat_size)
{
    int8_t in1 = *input_1;
    while (flat_size > 0)
    {
                                   ...
                                   const nmsis_nn_dims *output_dims)
{
    ...
    if (riscv_check_broadcast_required(input_1_dims, input_2_dims))
    {
        if (flat_size_1 == 1)
        {
            // riscv_max_scalar expects the tensor with the scalar value to be provided first
            riscv_max_scalar_s8(input_1_data, input_2_data, output_data, flat_size_2);
        }
        else if (flat_size_2 == 1)
        {
            // riscv_max_scalar expects the tensor with the scalar value to be provided first
            riscv_max_scalar_s8(input_2_data, input_1_data, output_data, flat_size_1);
        }
        else
        {
            ...
            for (int32_t i_out_batch = 0; i_out_batch < output_batch; i_out_batch++)
            {
                ...
                {
                    riscv_max_no_broadcast_s8(input_1_ptr, input_2_ptr, output_data, flat_size_1);
                    output_data += flat_size_1;
                }
                else if (flat_size_1 == 1)
                {
                    // riscv_max_scalar expects the tensor with the scalar value to be provided first
                    riscv_max_scalar_s8(input_1_ptr, input_2_ptr, output_data, flat_size_2);
                    output_data += flat_size_2;
                }
                else if (flat_size_2 == 1)
                {
                    // riscv_max_scalar expects the tensor with the scalar value to be provided first
                    riscv_max_scalar_s8(input_2_ptr, input_1_ptr, output_data, flat_size_1);
                    output_data += flat_size_1;
                }
                else
                {
                    ...
                    for (int32_t i_out_height = 0; i_out_height < output_height; i_out_height++)
                    {
                        ...
                        else
                        {
                            for (int32_t i_out_width = 0; i_out_width < output_width; i_out_width++)
                            {
                                ...
                                {
                                    riscv_max_no_broadcast_s8(input_1_ptr, input_2_ptr, output_data, input_1_channels);
                                ...
                                {
                                    // riscv_max_scalar expects the tensor with the scalar value to be provided first
                                    riscv_max_scalar_s8(input_1_ptr, input_2_ptr, output_data, input_2_channels);
                                ...
                                {
                                    // riscv_max_scalar expects the tensor with the scalar value to be provided first
                                    riscv_max_scalar_s8(input_2_ptr, input_1_ptr, output_data, input_1_channels);
                                    output_data += input_1_channels;
                                    input_1_ptr += input_1_channels;
                                    input_2_ptr++;
                                }
    ...
    {
        riscv_max_no_broadcast_s8(input_1_data, input_2_data, output_data, flat_size_1);
    }
