 ...
 * Title:        riscv_transpose_conv_wrapper_s8.c
...
riscv_nmsis_nn_status riscv_transpose_conv_wrapper_s8(const nmsis_nn_context *ctx,
                                                  const nmsis_nn_context *reverse_conv_ctx,
                                                  ...
                                                  int8_t *output_data)
{
    ...
    if (reverse_conv_possible && reverse_conv_efficient)
    {
        ...
        const nmsis_nn_dims transposed_input_dims = {input_n, input_h * stride_h, input_w * stride_w, input_c};
        const nmsis_nn_dims upscale_dims = {0, stride_h, stride_w, 0};
        ...
        for (int32_t i = 0; i < output_c; i++)
        {
            for (int32_t y = 0; y < filter_h; y++)
            {
                ...
                {
                    out_ptr -= input_c;
                    riscv_memcpy_s8(out_ptr, in_ptr, input_c * sizeof(int8_t));
                    in_ptr += input_c;
                }
            }
    ...
    else
    {
        ...
        return riscv_transpose_conv_s8(ctx,
                                     reverse_conv_ctx,
