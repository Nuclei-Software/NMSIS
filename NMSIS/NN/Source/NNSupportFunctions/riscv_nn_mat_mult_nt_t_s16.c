    ...
    for (; i_items <= (lhs_rows - 4); i_items += 4)
    {
        for (int i = 0; i < rhs_rows; i++)
        {
            ...
            if (is_int32_bias)
            {
                ...
                result = riscv_requantize_mve(res, dst_multipliers[i], dst_shifts[i]);
            ...
            else
            {
                ...
                acc_n0 = riscv_nn_requantize_s64(acc_n0_s64, reduced_multiplier, shift);
                acc_n1 = riscv_nn_requantize_s64(acc_n1_s64, reduced_multiplier, shift);
                acc_n2 = riscv_nn_requantize_s64(acc_n2_s64, reduced_multiplier, shift);
                acc_n3 = riscv_nn_requantize_s64(acc_n3_s64, reduced_multiplier, shift);
                int32x4_t res = {acc_n0, acc_n1, acc_n2, acc_n3};
        ...
        for (; i_items < lhs_rows; i_items++)
        {
            ...
            for (int i = 0; i < rhs_rows; i++)
            {
                ...
                if (index == 3)
                {
                    ...
                    res = riscv_requantize_mve_32x4(res, vldrwq_s32(multipliers), vldrwq_s32(shifts));
                    multipliers += 4;
                    shifts += 4;
                    res = vmaxq_s32(res, vdupq_n_s32(activation_min));
                    res = vminq_s32(res, vdupq_n_s32(activation_max));
            ...
            for (int i = 0; i < tail_rows; i++)
            {
                ...
                acc_n0 = riscv_nn_requantize(acc_n0, multipliers[i], shifts[i]);
